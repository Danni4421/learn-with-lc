...
/** Schemas */
import { StoreProgramSchema } from "@/schemas/program";

/** Server Action */
import { store } from "@/actions/programs";

export default function StoreProgramForm() {
  ...
  const {
    ...
    reset,
    setValue,
  } = useForm<z.infer<typeof StoreProgramSchema>>({
    defaultValues: {
      name: "",
      description: "",
    },
    resolver: zodResolver(StoreProgramSchema),
  });

  const onSubmit: SubmitHandler<z.infer<typeof StoreProgramSchema>> = (
    data
  ) => {
    setError("");
    setSuccess("");
