...
/** Server Action */
import { update } from "@/actions/teachers";
...
export default function EditTeacherForm({ teacher }: EditTeacherFormProps) {
  const {
    ...
    defaultValues: {
      ...
      role: teacher.role,
      last_graduate_at: teacher.last_graduate_at,
    },
  ...
  const onSubmit: SubmitHandler<z.infer<typeof PutTeacherSchema>> = (data) => {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        const response = await update(formData, teacher.id);
      ...
      >
        ...
        <Flex gap="2" mt="6">
          <WarningButton type="submit" disabled={isPending}>
            <FaRegFloppyDisk />
            <span className="ms-2">Ubah pengajar</span>
          </WarningButton>
